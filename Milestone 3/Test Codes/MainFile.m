...
Robot.number = 3; %number of mobile robots
...
Robot.rf = 4; %cost function parameter
...
for i = 1:Map.numberofObstacles
    a = repmat(Map.locationofObstacles(:,i)',4,1);
    b = [100 100;100 0;0 100;0 0];
    diffBetweenPointsAndEdges = a - b;
    Map.farthestPointFromObstacle(i) = max(cellfun(@norm,num2cell(diffBetweenPointsAndEdges,2)));
end

Figures.runningFigure = figure;
Figures.runningFigure.WindowState = 'maximized';

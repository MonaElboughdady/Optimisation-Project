function [Figures] = plotPathsPSO(Figures,Robot,Map,pos,PSO)
...
if isempty(numberOfExecutions)
    ...
    subplot(1,2,2);
    Figures.fitness = plot(PSO.bestFitnesses);
    legend("Cost")
    title('Cost of best particle')
    xlim([0 PSO.maxNumofIterations])
    ylim([-2 2])
    drawnow
end
subplot(1,2,1);
...
set(Figures.fitness,'YData',PSO.bestFitnesses)
drawnow
end